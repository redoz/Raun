...
/// <summary>The outcome of safely parsing one scenario: the parsed <see cref="Scenario"/>; or the
/// <see cref="Diagnostics"/> that kept it from lowering; or an <see cref="Error"/> (exception text)
/// plus the originating method's <see cref="File"/>/<see cref="Line"/> to report as RAUN000.</summary>
internal readonly record struct ScenarioResult(
    ParsedScenario? Scenario, EquatableArray<ScenarioDiagnostic> Diagnostics, string? Error, string? File, int Line);
...
internal static class GeneratorSafety
{
    public static ScenarioResult SafeParse(Func<ParseOutcome> parse, string? file, int line)
    {
        ...
        {
            var outcome = parse();
            return new ScenarioResult(outcome.Scenario, outcome.Diagnostics, null, null, 0);
        }
        catch (Exception ex)
        {
            return new ScenarioResult(null, EquatableArray<ScenarioDiagnostic>.Empty, Describe(ex), file, line);
        }
    }
